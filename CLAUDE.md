...
- **Relations** (`db.py`): `raw_data`, `codex_raw_messages`, `raw_messages`, `project_map`, `logical_sessions`, `assistant_message_costs`, `tool_calls`, `session_messages_enriched`, `conversation_turns`, `session_titles`, `message_commands`, `session_context_loads`, `file_reads`, `file_writes`, `session_stats`, `search_corpus`, `materialize_meta`
