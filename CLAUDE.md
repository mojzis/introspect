...
- `mcp/` — FastMCP tools + prompts over stdio or mounted on FastAPI; `_register.py` wires the query-template registry; `refresh_bridge.py` exposes the embedded `app.state` or standalone lifecycle state to stateless tools. Standalone startup loads in the background; data tools wait for the first snapshot and mark results partial until ready
...
- **Materialization**: `materialize_views()` runs on web and standalone MCP startup and rebuilds derived tables (incl. `session_stats`, `assistant_message_costs`, `session_messages_enriched`, `session_context_loads`); CLI commands call `ensure_materialized()` so they share the on-disk DB
